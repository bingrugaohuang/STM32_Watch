...
static SemaphoreHandle_t OLED_MutexHandle = NULL;
static uint8_t OLED_Contrast = 0xCF;

//OLED I2C总线锁定和解锁函数，使用FreeRTOS的递归互斥锁实现线程安全的I2C访问
void OLED_I2C_Lock(void) {
	//在调用锁定函数之前，先检查互斥锁句柄是否有效，并且FreeRTOS调度器已经启动，避免在调度器未启动时调用导致死锁
	if(OLED_MutexHandle && xTaskGetSchedulerState() != taskSCHEDULER_NOT_STARTED) {
		xSemaphoreTakeRecursive(OLED_MutexHandle, portMAX_DELAY);
	}
}

void OLED_I2C_Unlock(void) {
	if(OLED_MutexHandle && xTaskGetSchedulerState() != taskSCHEDULER_NOT_STARTED) {
		xSemaphoreGiveRecursive(OLED_MutexHandle);
	}
}
...
void OLED_WriteCommand(uint8_t Command)
{
	OLED_I2C_Lock();
	uint8_t buffer[2];
	buffer[0] = OLED_CTRL_CMD;
	buffer[1] = Command;
	(void)HAL_I2C_Master_Transmit(&hi2c1, OLED_I2C_ADDR, buffer, 2, OLED_I2C_TIMEOUT_MS);
	OLED_I2C_Unlock();
}
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
	...
	(void)HAL_I2C_Master_Transmit(&hi2c1, OLED_I2C_ADDR, tx, (uint16_t)(Count + 1), OLED_I2C_TIMEOUT_MS);
	OLED_I2C_Unlock();
}
...
void OLED_Init(void)
{
	if(OLED_MutexHandle == NULL) {
		OLED_MutexHandle = xSemaphoreCreateRecursiveMutex();
	}
	...
	OLED_WriteCommand(OLED_Contrast);	//0x00~0xFF
...
void OLED_ShowChar(int16_t X, int16_t Y, char Char, uint8_t FontSize)
{
	...
	{
		/*将ASCII字模库OLED_F6x8的指定数据以12*24的图像格式显示*/
...
void OLED_ShowString(int16_t X, int16_t Y, char *String, uint8_t FontSize)
{
	...
	while (*p != '\0')
	{
		if (*p < 0x80)
		{
			...
			OLED_ShowChar(X + XOffset, Y, ascii, FontSize);
			XOffset += FontSize;
			p++;
			continue;
		}
		...
		if (FontSize == OLED_8X16)
		{
			...
			if (p[1] != '\0')
			{
				...
				for (i = 0; strcmp(OLED_CF16x16[i].Index, "") != 0; i++)
				{
					if (strcmp(OLED_CF16x16[i].Index, key) == 0)
					{
						OLED_ShowImage(X + XOffset, Y, 16, 16, OLED_CF16x16[i].Data);
						XOffset += 16;
						p += 2;
						matched = 1;
						break;
			...
			{
				for (i = 0; i < (uint16_t)(sizeof(Utf8Map) / sizeof(Utf8Map[0])); i++)
				{
					if (p[0] == Utf8Map[i].Utf8[0] && p[1] == Utf8Map[i].Utf8[1] && p[2] == Utf8Map[i].Utf8[2])
					{
						OLED_ShowImage(X + XOffset, Y, 16, 16, OLED_CF16x16[Utf8Map[i].GlyphIndex].Data);
						XOffset += 16;
						p += 3;
						matched = 1;
						break;
...
//				{
//					OLED_DisplayBuf[Page + j][X + i] |= value << Shift;
//				}
//				if (Page + j + 1 >= 0 && Page + j + 1 <= 7)
//				{
//					OLED_DisplayBuf[Page + j + 1][X + i] |= value >> (8 - Shift);
//				}
//			}
//		}
//	}
...
//{
//	int16_t X = OLED_CompatColumnToX(Column, FontSize);
//	int16_t Y = OLED_CompatLineToY(Line, FontSize);

//	if (FontSize == OLED_8X16)
//	{
//		OLED_ShowImageWithReverse(X, Y, 8, 16, OLED_F8x16[Char - ' '], mode);
//	}
//	else if (FontSize == OLED_12X24)
//	{
//		OLED_ShowImageWithReverse(X, Y, 12, 24, OLED_F12x24[Char - ' '], mode);
//	}
//	else
//	{
//		OLED_ShowImageWithReverse(X, Y, 6, 8, OLED_F6x8[Char - ' '], mode);
//	}
...
//	{
//		OLED_ShowCharWithReverse(Line, (uint8_t)(Column + i), String[i], FontSize, mode);
//	}
//	return mode;
//}
...
//	for (i = 0; i < Length; i++)
//	{
//		OLED_ShowCharWithReverse(Line, (uint8_t)(Column + i), Number / OLED_Pow(10, Length - i - 1) % 10 + '0', FontSize, mode);
//	}
//	return mode;
//}
...
//	if (Number >= 0)
//	{
//		OLED_ShowCharWithReverse(Line, Column, '+', FontSize, mode);
//		Number1 = Number;
//	}
//	else
//	{
...
//	for (i = 0; i < Length; i++)
//	{
//		OLED_ShowCharWithReverse(Line, (uint8_t)(Column + i + 1), Number1 / OLED_Pow(10, Length - i - 1) % 10 + '0', FontSize, mode);
//	}
...
//		if (SingleNumber < 10)
//		{
//			OLED_ShowCharWithReverse(Line, (uint8_t)(Column + i), SingleNumber + '0', FontSize, mode);
//		}
//		else
//		{
//			OLED_ShowCharWithReverse(Line, (uint8_t)(Column + i), SingleNumber - 10 + 'A', FontSize, mode);
//		}
//	}
//	return mode;
//}
...
//	for (i = 0; i < Length; i++)
//	{
//		OLED_ShowCharWithReverse(Line, (uint8_t)(Column + i), Number / OLED_Pow(2, Length - i - 1) % 2 + '0', FontSize, mode);
//	}
//	return mode;
//}
...
//{
//	int16_t X = (int16_t)((Column - 1) * 16);
//	int16_t Y = (int16_t)((Line - 1) * 16);
//	return OLED_ShowImageWithReverse(X, Y, 16, 16, OLED_F10x16[Num], mode);
//}

//uint8_t OLED_ShowBMP(uint8_t x, uint8_t y, uint8_t width, uint8_t height, const uint8_t *bmp, uint8_t mode)
//{
//	int16_t pixelY = (int16_t)(y * 8);
//	return OLED_ShowImageWithReverse(x, pixelY, width, height, bmp, mode);
//}
...
//			if (temp & (1 << b))
//			{
//				OLED_DrawPointMode(X + i, Y + b, 1);
//			}
//			else
//			{
//				OLED_DrawPointMode(X + i, Y + b, 0);
//			}
//		}
//	}
//}
...
//		OLED_ShowCharPixel(X + (i * 6), Y, (char)(single_num + '0'));
//	}
//}
