...
static SemaphoreHandle_t MPU6050_MutexHandle = NULL;

// MPU6050 I2C总线锁定和解锁函数，使用FreeRTOS的递归互斥锁实现线程安全的I2C访问
void MPU6050_I2C_Lock(void)
{
    if (MPU6050_MutexHandle != NULL)
    {
        xSemaphoreTakeRecursive(MPU6050_MutexHandle, portMAX_DELAY);
    }
}
...
void MPU6050_I2C_Unlock(void)
{
    if (MPU6050_MutexHandle != NULL)
    {
        xSemaphoreGiveRecursive(MPU6050_MutexHandle);
    }
}
...
{
	MPU6050_MutexHandle = xSemaphoreCreateRecursiveMutex();
