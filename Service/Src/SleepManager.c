...
static const char *SleepMgr_StateName(SleepState_t state)
{
    switch(state)
    {
        case SLEEP_STATE_ACTIVE:        return "ACTIVE";
        case SLEEP_STATE_PREPARE_SLEEP: return "PREPARE_SLEEP";
        case SLEEP_STATE_SLEEPING:      return "SLEEPING";
        case SLEEP_STATE_PREPARE_STOP:  return "PREPARE_STOP";
        case SLEEP_STATE_STOP:          return "STOP";
        case SLEEP_STATE_WAKEUP:        return "WAKEUP";
...
static void SleepMgr_Init(TickType_t SleepTimeoutTicks, TickType_t StopTimeoutTicks)
{
    ...
    gLastActiveTick = OSAL_GetTickCount();
    ...
    gMotionWakeArmed = 0U;
    gStopContext = 0U;
    LOG_I("SLEEP", "Init sleep=%lu stop=%lu ticks",
          (unsigned long)gSleepTimeoutTicks,
          (unsigned long)gStopTimeoutTicks);
}
...
static void SleepMgr_EnterStopAndRecover(void)
{
    ...
    gStopContext = 1U;
    LOG_I("SLEEP", "Entering STOP mode");
    ...
    LOG_I("SLEEP", "STOP wake event received");
}
...
void SleepManager_Task(void *argument)
{
  ...
  TickType_t now = 0;
        ...
        SleepState_t lastState = gSleepState;
        LOG_I("SLEEP", "Sleep manager task started");
    
  while(1)
  {
        if(lastState != gSleepState)
        {
                LOG_I("SLEEP", "State %s -> %s",
                            SleepMgr_StateName(lastState),
                            SleepMgr_StateName(gSleepState));
                lastState = gSleepState;
        }

    switch (gSleepState)
    {
        case SLEEP_STATE_ACTIVE:
            now = OSAL_GetTickCount();
        ...
        case SLEEP_STATE_PREPARE_SLEEP:
            ...
            gSleepEnterTick = OSAL_GetTickCount(); // 记录进入睡眠状态的时间
        ...
        case SLEEP_STATE_SLEEPING:
            if(gWakeupPending) // 如果有待处理的唤醒事件
            {
                ...
                LOG_D("SLEEP", "Wake pending consumed in SLEEPING");
                break;
            }
            else
            {
        ...
        case SLEEP_STATE_WAKEUP:
            gLastActiveTick = OSAL_GetTickCount(); // 更新最后活跃时间
            ...
            if(gStopContext != 0U)
            {
                ...
                MX_I2C2_Init();
                gStopContext = 0U;
                LOG_I("SLEEP", "Recovered clocks and I2C after STOP");
            }
            ...
            if(Alarm_ServiceIsRinging() != 0U)
            {
                ...
                Alarm_SetMark(mark);
                LOG_I("SLEEP", "Alarm ringing, skip foreground resume");
                break;
            }
            ...
            LOG_I("SLEEP", "Foreground tasks resumed");
            break;
    }
...
void SleepMgr_ReportActivity(void)
{
    ...
    if(wakeRequested != 0U)
    {
        LOG_D("SLEEP", "Activity requested wakeup");
    }
}
...
{
    configASSERT((GPIO_Pin == KEY_CONFIRM_Pin) ||
                 (GPIO_Pin == KEY_NEXT_Pin) ||
                 (GPIO_Pin == KEY_LAST_Pin) ||
                 (GPIO_Pin == MPU_INT_Pin));

    gLastActiveTick = OSAL_GetTickCountFromISR(); // 更新最后活跃时间
    ...
    if((GPIO_Pin == KEY_CONFIRM_Pin) || (GPIO_Pin == KEY_NEXT_Pin) || (GPIO_Pin == KEY_LAST_Pin))
    {
        if((gSleepState == SLEEP_STATE_SLEEPING) ||
           ...
           (gSleepState == SLEEP_STATE_STOP))
        {
            gDiscardWakeKey = 1U; // 设置丢弃按键事件标志，避免刚醒来就被按键抖动立刻拉回睡眠
        }
    }
