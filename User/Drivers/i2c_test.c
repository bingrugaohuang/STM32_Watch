...
/* ================== 静态变量 ================== */
// static const I2C_Driver_t *i2c1 = NULL; // 已废弃，直接在 OLED_Init 中获取软件 I2C1 驱动实例
...
{
    LOG_I(TAG_TEST, "==== OLED functional test start ====");

    /* ---- Test 1: 全屏填充 ---- */
    LOG_I(TAG_TEST, "[Test 1] Full screen fill -> clear -> refresh");
    OLED_Clear();
    ...
    /* ---- Test 2: ASCII 文本 ---- */
    LOG_I(TAG_TEST, "[Test 2] ASCII text display");
    ...
    LOG_I(TAG_TEST, "[Test 3] Chinese text display");
    OLED_Clear();
    ...
    /* ---- Test 4: 数字显示 ---- */
    LOG_I(TAG_TEST, "[Test 4] Number display");
    OLED_Clear();
    ...
    /* ---- Test 5: 浮点数显示 ---- */
    LOG_I(TAG_TEST, "[Test 5] Float number display");
    OLED_Clear();
    ...
    /* ---- Test 6: 输出格式显示 ---- */
    LOG_I(TAG_TEST, "[Test 6] OLED_Printf formatted output");
    OLED_Clear();
    ...
    /* ---- Test 7: 图形绘制 ---- */
    LOG_I(TAG_TEST, "[Test 7] Shape drawing");
    OLED_Clear();
    ...
    /* ---- Test 8: 进度条动画 ---- */
    LOG_I(TAG_TEST, "[Test 8] Progress bar animation");
    OLED_Clear();
    ...
    /* ---- Test complete ---- */
    LOG_I(TAG_TEST, "[Test 9] Final screen - test complete");
    OLED_Clear();
    ...
    LOG_I(TAG_TEST, "==== OLED functional test complete ====");
    uint32_t tick = (uint32_t)osal_get_tick();
    LOG_I(TAG_TEST, "Test finished at tick: %lu (~%lu seconds)", tick, tick / 1000);
}
...
{
    (void)pvParameters;
    LOG_I(TAG_TEST, "I2C test task started");
    ...
    while (1)
    {
        ...
        LOG_D(TAG_TEST, "Heartbeat: %lu", counter);
...
{
    //已经弃用的I2C初始化代码，已移至OLED_Init函数
//     LOG_I(TAG_TEST, "Initializing I2C subsystem...");

//     i2c1 = (I2C_Driver_t *)I2C1_SW_GetDriver();
// #if I2C1_SW_ENABLE
//     i2c1->init();
//     LOG_I(TAG_TEST, "SW I2C1 initialized (PB6=SCL, PB7=SDA)");
// #else
//     MX_I2C1_Init();
//     LOG_I(TAG_TEST, "HW I2C1 initialized");
// #endif
    
    test_handle = osal_task_create("I2C_OLED_test", I2C_SW_test,
                                    256, NULL, 3);
    if (test_handle == NULL)
        LOG_E(TAG_TEST, "I2C test task creation failed!");
    else
        LOG_I(TAG_TEST, "I2C test task created (stack=256 words, priority=3)");
}

#endif /* I2C1_OLED_TEST_ENABLE */